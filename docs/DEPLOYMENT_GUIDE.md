...
| 环境 | 文件路径 |
|-----|---------|
| **开发环境** | `d:\fangtang\ciliAI\fangtang.db` |
| **生产环境** | `/var/www/ciliAI/fangtang.db` |
...
# 步骤2：备份现有数据库
cp fangtang.db ciliAI_backup_$(date +%Y%m%d).db
...
```bash
# 备份命令
cp fangtang.db ciliAI_backup_$(date +%Y%m%d_%H%M%S).db

# 压缩备份
tar -czf ciliAI_backup_$(date +%Y%m%d).tar.gz fangtang.db
```
...
DB_FILE="/var/www/ciliAI/fangtang.db"
...
# 步骤2：备份当前数据库
cp fangtang.db ciliAI_current.db
...
# 解压
tar -xzf /tmp/ciliAI_20260412.tar.gz -C /var/www/ciliAI/
...
Environment="PATH=/var/www/ciliAI/venv/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin"
Environment="PYTHONUNBUFFERED=1"
ExecStart=/var/www/ciliAI/venv/bin/gunicorn \
    --bind 0.0.0.0:5001 \
    --workers 4 \
    --timeout 120 \
    --access-logfile /var/log/ciliAI/access.log \
...
# 验证数据库
echo -n "检查数据库... "
if sqlite3 /var/www/ciliAI/fangtang.db "SELECT COUNT(*) FROM users;" > /dev/null 2>&1; then
    echo -e "${GREEN}✓ 通过${NC}"
else
    echo -e "${RED}✗ 失败${NC}"
fi
...
1. 检查是否有其他进程访问数据库
```bash
lsof /var/www/ciliAI/fangtang.db
```
...
cp $SOURCE_DIR/ciliAI/fangtang.db $BACKUP_DIR/fangtang.db.$DATE
...
# 步骤5：恢复文件
cp /tmp/fangtang.db /var/www/ciliAI/
cp /tmp/.env /var/www/ciliAI/

# 步骤6：验证数据
sqlite3 /var/www/ciliAI/fangtang.db "SELECT COUNT(*) FROM users;"
