...
<script setup>
import { ref, reactive, computed, onMounted, nextTick, inject, triggerRef } from 'vue'
...
const loadChatSessions = async () => {
  ...
  try {
    ...
    if (result.status === 'success') {
      chatList.value = result.sessions.map(session => reactive({
...
const loadChatMessages = async (chatId) => {
  ...
  try {
    const response = await fetch(`/api/chat/messages?invite_code=${encodeURIComponent(inviteCode)}&chat_id=${chatId}`)
    const result = await response.json()
...
const sendMessage = async () => {
  ...
  let chat = chatList.value.find(c => c.id === currentChatId.value)
  if (!chat) {
    await createNewChat()
    chat = chatList.value.find(c => c.id === currentChatId.value)
  }
          ...
          if (trimmedLine.startsWith('data: ')) {
            ...
            try {
              ...
              if (data.event === 'message' || data.event === 'agent_message') {
                ...
                result += answerText
                console.log('📤 准备调用 onChunk，内容长度:', answerText.length)
                if (onChunk) {
                  onChunk(answerText, data.conversation_id || newConversationId)
                  console.log('✅ onChunk 已调用')
                } else {
                  console.warn('⚠️ onChunk 未定义')
                }
              }
              
              if (data.event === 'agent_thought') {
                console.log('Agent 思考:', data.thought || data)
              }
              
              if (data.event === 'text_chunk' || data.event === 'text') {
                const answerText = data.text || data.answer || ''
                if (answerText) {
                  result += answerText
                  if (onChunk) {
                    onChunk(answerText, data.conversation_id || newConversationId)
