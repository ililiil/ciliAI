...
def process_base64_image(image_data, prefix='image'):
    """
    处理Base64图片数据，如果是data URL则保存为文件并返回路径
    
    Args:
        image_data: 图片数据，可能是Base64字符串或文件路径
        prefix: 文件名前缀，用于区分不同类型的图片
    
    Returns:
        str: 处理后的图片路径（可能是原路径或新文件路径）
    """
    if not image_data:
        return ''
    
    # 如果不是Base64 data URL，直接返回
    ...
    try:
        ...
        filename = f"{prefix}_{timestamp}_{random_suffix}.{file_ext}"
        ...
        logger.info(f"图片已保存: {filename} ({len(file_data)} bytes)")
...
def execute_query(query, params=None, fetch_one=False, fetch_all=False):
    """
    统一执行数据库查询的辅助函数，自动处理 MySQL 和 SQLite 的差异
    ...
    Returns:
        如果 fetch_one=True: 返回单条记录或 None
        如果 fetch_all=True: 返回记录列表
        否则: 返回 cursor
    """
    db = get_db()
    cursor = db.cursor()
...
def init_db():
    ...
    try:
        ...
        pass
    try:
        cursor.execute('ALTER TABLE ip_works ADD COLUMN user_id INTEGER')
    except:
        pass
    try:
        cursor.execute('ALTER TABLE ip_works ADD COLUMN qr_code TEXT')
    except:
        pass
    ...
    for code in default_codes:
        try:
            cursor.execute('INSERT OR IGNORE INTO invite_codes (code, compute_power) VALUES (%s, %s)', (code, 1000))
        except:
            pass
        ...
        default_ip_works = [
            {
                ...
                'image': 'https://picsum.photos/400/600%srandom=10',
                'tags': '["AI动画","现代科幻","穿越"]',
                'cost': '1000算力/分钟',
                'duration': '420分钟',
                'price': '150-300元/分钟',
            ...
            },
            {
                'title': '古风仙侠传',
                'student_name': 'CiliAI官方',
                'image': 'https://picsum.photos/400/600%srandom=11',
                'tags': '["AI动画","古风","仙侠"]',
                'cost': '1200算力/分钟',
                'duration': '600分钟',
                'price': '180-350元/分钟',
            ...
            },
            {
                'title': '都市悬疑夜',
                'student_name': 'CiliAI官方',
                'image': 'https://picsum.photos/400/600%srandom=12',
                'tags': '["AI动画","都市","悬疑"]',
                'cost': '1100算力/分钟',
                'duration': '480分钟',
                'price': '160-320元/分钟',
            ...
            },
            {
                ...
                'image': 'https://picsum.photos/400/600%srandom=13',
                'tags': '["AI动画","创意","实验"]',
                'cost': '800算力/分钟',
                'duration': '300分钟',
                'price': '100-200元/分钟',
            ...
            },
            {
                'title': 'AI创作者作品2',
                'student_name': '创意达人小红',
                'image': 'https://picsum.photos/400/600%srandom=14',
                'tags': '["AI动画","现代","情感"]',
                'cost': '900算力/分钟',
                'duration': '360分钟',
                'price': '120-250元/分钟',
            ...
            },
            {
                'title': 'AI创作者作品3',
                'student_name': '技术宅阿杰',
                'image': 'https://picsum.photos/400/600%srandom=15',
                'tags': '["AI动画","科幻","未来"]',
                'cost': '950算力/分钟',
                'duration': '400分钟',
                'price': '140-280元/分钟',
...
def get_user_by_invite_code(invite_code):
    if DB_TYPE == 'mysql':
        return execute_query('SELECT * FROM users WHERE invite_code = %s', (invite_code,), fetch_one=True)
    else:
        return execute_query('SELECT * FROM users WHERE invite_code = %s', (invite_code,), fetch_one=True)
...
def create_user(invite_code, compute_power=0):
    ...
    cursor = db.cursor()
    if DB_TYPE == 'mysql':
        cursor.execute('INSERT INTO users (invite_code, compute_power) VALUES (%s, %s)', (invite_code, compute_power))
    else:
        cursor.execute('INSERT INTO users (invite_code, compute_power) VALUES (%s, %s)', (invite_code, compute_power))
    db.commit()
...
def get_user_compute_power(user_id):
    if DB_TYPE == 'mysql':
        user = execute_query('SELECT compute_power FROM users WHERE id = %s', (user_id,), fetch_one=True)
    else:
        user = execute_query('SELECT compute_power FROM users WHERE id = %s', (user_id,), fetch_one=True)
...
def deduct_compute_power(user_id, amount, operation_type, project_id=None, record_id=None, description=None):
    ...
    if DB_TYPE == 'mysql':
        cursor.execute('UPDATE users SET compute_power = %s, total_power_used = total_power_used + %s, last_active = %s WHERE id = %s', (new_power, amount, datetime.now(), user_id))
        cursor.execute('''
    ...
    if project_id:
        cursor = db.cursor()
        cursor.execute('UPDATE projects SET total_power_used = total_power_used + %s, update_time = %s WHERE id = %s', (amount, datetime.now(), project_id))
...
def add_compute_power(user_id, amount, operation_type, description=None):
    ...
    cursor = db.cursor()
    cursor.execute('UPDATE users SET compute_power = %s, last_active = %s WHERE id = %s', (new_power, datetime.now(), user_id))
...
def verify_invite_code():
    ...
    cursor = db.cursor()
    cursor.execute('UPDATE invite_codes SET status = %s, used_at = %s, current_uses = current_uses + 1 WHERE code = %s', ('used', datetime.now(), invite_code))
    db.commit()
...
def get_power_logs():
    ...
    cursor = db.cursor()
    cursor.execute('SELECT COUNT(*) as count FROM compute_power_logs WHERE user_id = %s', (user_id,))
...
def get_project(project_id):
    ...
    return jsonify({
        'status': 'success',
        'project': project if isinstance(project, dict) else dict(project),
...
def generate_image():
    ...
    logger.info(f"收到生图请求 - prompt: {prompt}, invite_code: {invite_code}, project_id: {project_id}, has_reference: {bool(reference_image)}")
    ...
    if reference_image:
        params["binary_data_base64"] = [extract_base64(reference_image)]
        ...
        for i in range(max_retries):
            ...
            if resp_code == 10000:
                ...
                if status == 'done':
                    ...
                    # 存储精简版参数，不包含base64图片数据
                    storage_params = {
                        "req_key": params.get("req_key"),
                    ...
                    ''', (project_id, user_id, 'generate', prompt, 
                          ...
                          'v4.0', json.dumps(storage_params), task_id, power_cost))
                    record_id = cursor.lastrowid
                    
                    if project_id:
                        cursor = db.cursor()
                        cursor.execute('UPDATE projects SET image_count = image_count + 1, update_time = %s WHERE id = %s', (datetime.now(), project_id))
                    
                    cursor = db.cursor()
                    cursor.execute('''UPDATE compute_power_logs SET record_id = %s WHERE id = (
                        SELECT t.id FROM (
...
def inpaint_image():
    ...
    clean_image_base64 = extract_base64(image_base64)
    clean_mask_base64 = extract_base64(mask_base64)

    params = {
        "req_key": "jimeng_image2image_dream_inpaint",
        "binary_data_base64": [clean_image_base64, clean_mask_base64],
            ...
            if resp_code == 10000:
                ...
                if status == 'done':
                    ...
                    ''', (project_id, user_id, 'inpaint', prompt, 
                          image_urls[0] if image_urls else None,
                          json.dumps(storage_params), task_id, power_cost))
                    record_id = cursor.lastrowid
...
def extend_image():
    ...
    params = {
        ...
        "binary_data_base64": [extract_base64(image_base64)],
...
def get_record(record_id):
    db = get_db()
    cursor = db.cursor()
    cursor.execute('SELECT * FROM generation_records WHERE id = %s', (record_id,))
...
def delete_record(record_id):
    ...
    if record['project_id']:
        cursor = db.cursor()
        cursor.execute('UPDATE projects SET image_count = image_count - 1 WHERE id = %s', (record['project_id'],))
...
def save_chat_message():
    ...
    if data.get('project_id'):
        cursor = db.cursor()
        cursor.execute('UPDATE projects SET chat_count = chat_count + 1, update_time = %s WHERE id = %s', (datetime.now(), data.get('project_id')))
...
def get_stats():
    ...
    cursor = db.cursor()
    cursor.execute('SELECT COUNT(*) as count FROM projects WHERE user_id = %s AND status != "deleted"', (user['id'],))
    project_count = cursor.fetchone()['count']
    cursor = db.cursor()
    cursor.execute('SELECT COUNT(*) as count FROM generation_records WHERE user_id = %s', (user['id'],))
    image_count = cursor.fetchone()['count']
    cursor = db.cursor()
    cursor.execute('SELECT COUNT(*) as count FROM chat_messages WHERE user_id = %s', (user['id'],))
    ...
    ''', (user['id'], today_start))
    today_power_used = cursor.fetchone()['total']
...
def get_public_works():
    try:
        ...
        for idx, work in enumerate(works):
            work_dict = work if isinstance(work, dict) else work if isinstance(work, dict) else dict(work)
...
def update_work_image(work_id):
    ...
    try:
        ...
        cursor.execute('UPDATE ip_works SET image = %s, updated_at = %s WHERE id = %s', (image_url, datetime.now(), work_id))
        conn.commit()
        conn.close()
...
def get_featured_works():
    ...
    try:
        ...
        works_list = []
        for idx, work in enumerate(works):
            work_dict = work if isinstance(work, dict) else dict(work)
            image_url = work_dict.get('image', '').strip() if work_dict.get('image') else ''
...
def get_advertisements():
    ...
    try:
        ...
        ads_list = []
        for ad in ads:
            ad_dict = ad if isinstance(ad, dict) else dict(ad)
...
def get_admin_invite_codes():
    ...
    if DB_TYPE == 'mysql':
        cursor.execute('''
            SELECT ic.*, (SELECT COUNT(*) FROM users WHERE invite_code = ic.code) as use_count
    ...
    if DB_TYPE == 'mysql':
        codes_list = list(codes) if codes else []
    else:
        columns = [desc[0] for desc in cursor.description]
        codes_list = [dict(zip(columns, code)) for code in codes]
...
def create_admin_invite_code():
    logger.info("DEBUG: create_admin_invite_code called")
    import sys
    print("DEBUG: create_admin_invite_code called", flush=True)
    print(f"DEBUG: request.json = {request.json}", flush=True)
    sys.stdout.flush()
    try:
        data = request.json
        if data is None:
            logger.error("request.json is None")
            print("DEBUG: request.json is None", flush=True)
            return jsonify({'code': 400, 'msg': '无效的请求数据'}), 400

        code = data.get('code')
        compute_power = data.get('compute_power', 1000)
        print(f"DEBUG: code = {code}, compute_power = {compute_power}", flush=True)

        if not code:
            code = generate_invite_code(8)
            print(f"DEBUG: generated code = {code}", flush=True)

        if len(code) != 8:
            return jsonify({'code': 400, 'msg': '邀请码必须为8位字符'}), 400

        db = get_db()
        print("DEBUG: inserting into database", flush=True)
        cursor = db.cursor()
        if DB_TYPE == 'mysql':
            cursor.execute('INSERT INTO invite_codes (code, compute_power) VALUES (%s, %s)', (code.upper(), compute_power))
        else:
            cursor.execute('INSERT INTO invite_codes (code, compute_power) VALUES (%s, %s)', (code.upper(), compute_power))
        db.commit()
        ...
        print("DEBUG: IntegrityError", flush=True)
...
def update_admin_invite_code(id):
    ...
    cursor = db.cursor()
    cursor.execute('SELECT * FROM invite_codes WHERE id = %s', (id,))
    ...
    cursor = db.cursor()
    cursor.execute('UPDATE invite_codes SET status = %s, compute_power = %s WHERE id = %s', (status, compute_power, id))
    db.commit()
...
def delete_admin_invite_code(id):
    ...
    cursor = db.cursor()
    cursor.execute('DELETE FROM invite_codes WHERE id = %s', (id,))
    db.commit()
...
def batch_create_admin_invite_codes():
    ...
    while len(created_codes) < count and attempts < max_attempts:
        ...
        try:
            if DB_TYPE == 'mysql':
                cursor.execute('INSERT INTO invite_codes (code, compute_power) VALUES (%s, %s)', (code, compute_power))
            else:
                cursor.execute('INSERT INTO invite_codes (code, compute_power) VALUES (%s, %s)', (code, compute_power))
...
def get_admin_works():
    ...
    return jsonify({
        'code': 200,
        'data': {
            'list': [work if isinstance(work, dict) else dict(work) for work in works],
            'total': len(works)
        }
    })
...
def create_admin_work():
    ...
    cursor.execute('''
        INSERT INTO ip_works (user_id, title, student_name, image, qr_code, tags, cost, duration, price, copyright, introduction, category)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
    ''', (user_id, title, student_name, image, qr_code, json.dumps(tags), cost, duration, price, copyright, introduction, category))
    db.commit()
...
def update_admin_work(id):
    ...
    cursor = db.cursor()
    cursor.execute('SELECT * FROM ip_works WHERE id = %s', (id,))
    work = cursor.fetchone()
    if not work:
    ...
    for field in ['title', 'student_name', 'image', 'qr_code', 'tags', 'cost', 'duration', 'price', 'copyright', 'introduction', 'category', 'status']:
        if field in data:
    ...
    if update_fields:
        ...
        cursor = db.cursor()
        cursor.execute(f'UPDATE ip_works SET {", ".join(update_fields)} WHERE id = %s', update_values)
        db.commit()
...
def delete_admin_work(id):
    ...
    cursor = db.cursor()
    cursor.execute('DELETE FROM ip_works WHERE id = %s', (id,))
    db.commit()
...
def update_admin_order(order_id):
    try:
        ...
        cursor.execute('''
            UPDATE orders SET 
                title = %s, image = %s, qrcode = %s, price = %s, deadline = %s, 
                status = %s, tags = %s, contact_count = %s
            WHERE id = %s
...
def get_admin_users():
    ...
    return jsonify({
        'code': 200,
        'data': {
            'list': [user if isinstance(user, dict) else dict(user) for user in users],
            'total': len(users)
        }
    })
...
def get_admin_stats():
    ...
    cursor = db.cursor()
    cursor.execute('SELECT COUNT(*) as count FROM invite_codes WHERE status = "used"')
    used_code_count = cursor.fetchone()['count']
...
def get_admin_power_logs():
    try:
        ...
        if user_id:
            ...
            cursor = db.cursor()
            cursor.execute('SELECT COUNT(*) as count FROM compute_power_logs')
            total = cursor.fetchone()['count']
        
        cursor = db.cursor()
        cursor.execute('SELECT COALESCE(SUM(ABS(power_change)), 0) as total FROM compute_power_logs WHERE power_change < 0')
        total_power_used = cursor.fetchone()['total']
...
def update_admin_user_power(user_id):
    try:
        ...
        if operation == 'set':
            ...
            cursor = db.cursor()
            cursor.execute('UPDATE users SET compute_power = %s WHERE id = %s', (new_power, user_id))
        elif operation == 'add':
...
def update_chat_session(session_id):
    ...
    if 'selected_people' in data:
        updates.append('selected_people = %s')
    ...
    if updates:
        ...
        cursor = db.cursor()
        cursor.execute(f'UPDATE chat_sessions SET {", ".join(updates)} WHERE id = %s', values)
        db.commit()
...
def get_session_messages(session_id):
    ...
    cursor.execute('''
        ...
        ORDER BY create_time ASC
    ''', (user_id, session['conversation_id'] if session['conversation_id'] else ''))
    messages = cursor.fetchall()
