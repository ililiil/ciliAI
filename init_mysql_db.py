...
def init_mysql_database():
    ...
    try:
        ...
        migrations = [
            ("ALTER TABLE ip_works ADD COLUMN user_id INT NULL", "ip_works.user_id"),
            ("ALTER TABLE ip_works ADD COLUMN qr_code TEXT NULL", "ip_works.qr_code"),
        ]
        for sql, field in migrations:
            try:
                cursor.execute(sql)
                logger.info(f"✓ 字段迁移完成: {field}")
            except Exception as e:
                if "Duplicate column" in str(e) or "already exists" in str(e).lower():
                    pass
                else:
                    logger.warning(f"字段迁移警告: {field} - {e}")
