...
def check_frontend():
    ...
    if check_port(3003):
        print("✅ 前端服务端口 3003 正在监听")

        try:
            # 测试前端页面
            response = requests.get('http://localhost:3003', timeout=5, allow_redirects=False)
            print(f"✅ 前端页面响应正常 (状态码: {response.status_code})")
            print(f"✅ 前端服务运行正常")
            return True
        except Exception as e:
            print(f"❌ 无法连接到前端服务: {e}")
